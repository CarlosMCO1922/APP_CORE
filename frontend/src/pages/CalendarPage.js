...
import { FaArrowLeft, FaTimes, FaUsers, FaUserMd, FaExternalLinkAlt, FaCalendarPlus, FaInfoCircle, FaExclamationTriangle, FaVideo, FaImage } from 'react-icons/fa';
...
const CalendarWrapper = styled.div`
  ...
  .rbc-toolbar button.rbc-active {
    ...
    color: ${({ theme }) => theme.colors.textDark};
    border-color: ${({ theme }) => theme.colors.primary};
    box-shadow: 0 0 5px ${({ theme }) => theme.colors.primary}70;
  }
  ...
  .rbc-header {
    ...
    text-align: center;
    font-weight: 500;
    font-size: 0.8rem; /* Ligeiramente menor para mais espaço */
  ...
  .rbc-event, .rbc-day-slot .rbc-event {
    ...
    color: ${({ theme }) => theme.colors.textDark};
    border: none;
    border-radius: 4px; /* Menos arredondado para um look mais clean */
    padding: 3px 6px; /* Ajustado para caber melhor */
    font-size: 0.78rem; /* Ajustado */
    ...
    &:hover {
        ...
        transform: scale(1.02); /* Efeito de hover subtil */
  ...
  .rbc-event-content { // Tempo na visualização de mês
    display: none; // Muitas vezes polui, o título é suficiente
  }

  .rbc-event.rbc-selected {
    background-color: #b89b2e; 
    box-shadow: 0 0 0 2px ${({theme}) => theme.colors.background}, 0 0 0 4px ${({theme}) => theme.colors.primary}; // Destaque mais moderno
  }

  /* Estilos para a vista de Agenda */
  .rbc-agenda-view {
    table {
        border: 1px solid ${({ theme }) => theme.colors.cardBorder};
        thead th {
  ...
  .rbc-day-bg + .rbc-day-bg { border-left: 1px solid ${({ theme }) => theme.colors.cardBorder}; }
  .rbc-month-row + .rbc-month-row { border-top: 1px solid ${({ theme }) => theme.colors.cardBorder}; }
  
  .rbc-today {
    background-color: rgba(212, 175, 55, 0.08); // Mais subtil
    border: 1px solid ${({ theme }) => theme.colors.primary}30; // Borda subtil
  }
  ...
  .rbc-slot-selectable {
    cursor: pointer;
    &:hover {
      background-color: rgba(212, 175, 55, 0.05); // Hover mais subtil
    }
  }
  .rbc-current-time-indicator {
    background-color: ${({ theme }) => theme.colors.error};
    height: 2px;
    box-shadow: 0 0 5px ${({ theme }) => theme.colors.error};
    &::before {
...
`;
const PageErrorText = styled(MessageBase)` color: ${({ theme }) => theme.colors.error}; background-color: ${({ theme }) => theme.colors.errorBg}; border-color: ${({ theme }) => theme.colors.error};`;
const PageSuccessMessage = styled(MessageBase)` color: ${({ theme }) => theme.colors.success}; background-color: ${({ theme }) => theme.colors.successBg}; border-color: ${({ theme }) => theme.colors.success};`;
const LoadingText = styled(MessageBase)` color: ${({ theme }) => theme.colors.primary}; border-color: transparent; background: transparent;`;
const NoItemsContainer = styled.div` display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 30px 15px; background-color: ${({ theme }) => theme.colors.cardBackground}CC; border-radius: ${({ theme }) => theme.borderRadius}; text-align: center; color: ${({ theme }) => theme.colors.textMuted}; margin-top: 20px; svg { font-size: 2.5rem; color: ${({ theme }) => theme.colors.primary}99; margin-bottom: 10px; } p { font-size: 1rem; margin: 0; }`;
...
const ModalDetail = styled.p`
  margin: 10px 0; font-size: 0.95rem; line-height: 1.6;
  color: ${({ theme }) => theme.colors.textMuted};
  ...
  svg {
    ...
    font-size: 1.1em; margin-top: 3px;
  }
`;
...
const ModalErrorText = styled.PageErrorText`margin: 0 0 12px 0; text-align:left; font-size: 0.8rem;`;
...
const EventComponentStyled = styled.div`
  ...
  height: 100%;
  padding: 2px 0; /* Pequeno padding vertical interno */
  
  .event-icon {
    font-size: 0.9em; /* Tamanho do ícone relativo ao texto do evento */
    opacity: 0.85;
  ...
  .event-details-text {
    font-size: 0.8em; /* Detalhes um pouco menores */
    opacity: 0.75;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    margin-left: auto; /* Empurra para a direita se houver espaço */
    padding-left: 5px; /* Pequeno espaço antes dos detalhes */
    flex-shrink: 0;
  }
`;
...
const CustomEventComponent = ({ event }) => (
  <EventComponentStyled title={event.title + (event.resource.type === 'training' ? ` (${event.resource.participantsCount ?? event.resource.participants?.length ?? 0}/${event.resource.capacity})` : '')}>
    ...
    <span className="event-title-text">{event.title.split('(')[0].trim().split(':')[0]}</span> {/* Simplifica o título principal */}
    ...
    <PageContainer>
      ...
      <CalendarWrapper>
        <Calendar
          ...
          popup
          timeslots={1} // Número de slots por hora (1 = slots de 1 hora)
          step={60}     // Intervalo em minutos para os slots (60 min = 1 hora)
          min={new Date(0, 0, 0, 7, 0, 0)}  // Início do dia no calendário
          max={new Date(0, 0, 0, 22, 0, 0)} // Fim do dia no calendário
          formats={{
        ...
        <ModalOverlay onClick={handleCloseRequestModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            ...
            <RequestModalForm onSubmit={handleRequestSubmit}>
              ...
              <RequestModalInput type="time" name="time" id="reqTime" value={requestFormData.time} onChange={handleRequestFormChange} required step="1800" />
