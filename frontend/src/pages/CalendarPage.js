...
const localizer = dateFnsLocalizer({
  format,
  parse,
  startOfWeek: (date) => startOfWeek(date, { weekStartsOn: 1 }), // Semana começa à Segunda-feira
  getDay,
  locales,
});
...
const CalendarWrapper = styled.div`
  ...
  height: 80vh; /* Altura ajustável conforme necessidade */
  ...
  .rbc-event.rbc-selected { 
    background-color: #b89b2e; /* Um dourado mais escuro para seleção */
  }
  .rbc-agenda-event-cell, .rbc-time-slot, .rbc-day-slot .rbc-time-slot { 
  ...
  .rbc-today { 
    background-color: #3a3a3aAA; /* Cor de hoje com alguma transparência */
  }
`;
...
const ModalDetail = styled.p` 
  ...
  span { font-weight: bold; color: #D4AF37; }
`;
...
const ModalButton = styled.button` 
  background-color: ${props => props.danger ? '#D32F2F' : (props.primary ? '#D4AF37' : '#555')}; 
  color: ${props => props.danger ? 'white' : (props.primary ? '#1A1A1A' : '#E0E0E0')}; 
  padding: 10px 18px; border-radius: 6px; border: none; 
  cursor: pointer; font-weight: 500; 
  transition: background-color 0.2s ease; 
  &:hover { 
    background-color: ${props => props.danger ? '#C62828' : (props.primary ? '#e6c358' : '#666')}; 
  } 
  &:disabled { 
    background-color: #404040; color: #777; cursor: not-allowed; 
  } 
`;
const CloseButton = styled.button` 
  ...
  line-height: 1; padding: 0; /* Para melhor alinhamento do X */
  &:hover { color: #fff; } 
`;
...
const CalendarPage = () => {
  ...
  const handleRequestSubmit = async (e) => {
    ...
    } catch (err) { setRequestFormError(err.message || 'Falha ao enviar pedido de consulta.');
  ...
  const handleBookSelectedTraining = async () => {
    ...
    } catch (err) { setError(err.message || 'Falha ao inscrever no treino.');
  ...
  const handleCancelTrainingBooking = async () => {
    ...
    try {
      ...
      await fetchPageData(); handleCloseEventModal();
    } catch (err) { setError(err.message || 'Falha ao cancelar inscrição.');
    } finally { setActionLoading(false); }
  };
  ...
  const handleBookSelectedAppointment = async () => {
    ...
    try {
      ...
      await fetchPageData(); handleCloseEventModal();
    } catch (err) { setError(err.message || 'Falha ao marcar consulta.');
    } finally { setActionLoading(false); }
  };
  ...
  const handleCancelAppointmentBooking = async () => {
    ...
    try {
      ...
      await fetchPageData(); handleCloseEventModal();
    } catch (err) { setError(err.message || 'Falha ao cancelar consulta.');
    } finally { setActionLoading(false); }
  };
    ...
    <PageContainer>
      ...
      {showEventModal && selectedEvent && (
        <ModalOverlay onClick={handleCloseEventModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <CloseButton onClick={handleCloseEventModal}>&times;</CloseButton>
      ...
      {showRequestModal && authState.role === 'user' && (
        <ModalOverlay onClick={handleCloseRequestModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <CloseButton onClick={handleCloseRequestModal}>&times;</CloseButton>
